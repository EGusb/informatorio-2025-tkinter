...
class TodoApp(tk.Tk):
    ...
    def apply_theme(self):
        ...
        self.style.configure("Header.TLabel", background=theme["bg"],
                             foreground=theme["fg"], font=('Helvetica', 16, 'bold'))
        ...
        self.style.configure("TEntry", fieldbackground=theme["entry_bg"],
                             foreground=theme["entry_fg"], insertcolor=theme["fg"])
    ...
    def edit_task(self):
        ...
        nuevo_texto = simpledialog.askstring(
            "Editar Tarea", "Edita el texto de la tarea:", initialvalue=selected_task['texto'])
    ...
    def set_task_date(self):
        ...
        fecha_limite_str = simpledialog.askstring(
            "Asignar Fecha", "Ingresa la fecha límite (YYYY-MM-DD):", initialvalue=selected_task.get('fecha_limite', ''))
        if fecha_limite_str:
            try:
                datetime.strptime(fecha_limite_str, "%Y-%m-%d")
